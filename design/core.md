...
在产品战略上，Aetheris 正从云托管 SaaS 叙事转向 **B2D 企业级 AI 智能体底层运行时**：面向开发者与架构师，优先保证持久化执行、可审计与本地优先部署能力。
...
## 2. 总体架构分层（Eino-first + Runtime-first）

系统采用 **Eino 负责构建，Aetheris 负责执行** 的分层设计：
...
当前工程能力与战略四阶段已经形成一一对应关系：

| 阶段    | 目标                               | 当前状态         | 关键支撑                                      |
| ------- | ---------------------------------- | ---------------- | --------------------------------------------- |
| Phase 1 | 轻量级本地事件/状态存储            | 已落地           | embedded stores 与 local-first 模式           |
| Phase 2 | Durable Execution API（中断/恢复） | 已落地           | wait/signal、checkpoint、replay、exact resume |
| Phase 3 | MCP 标准化接入                     | 已落地           | MCP host + tool adapter 抽象                  |
| Phase 4 | Open Core 商业模式                 | 已定义并落地边界 | OSS runtime 与 Enterprise 增强能力分层        |

详见 [docs/strategy-and-user-stories.md](../docs/strategy-and-user-stories.md)。
...
- [internal/effect-system.md](internal/effect-system.md) — Effect 类型与 Replay 协议
- [internal/scheduler-correctness.md](internal/scheduler-correctness.md) — 调度器正确性
- [internal/runtime-contract.md](internal/runtime-contract.md) — 运行时契约
- [internal/step-contract.md](internal/step-contract.md) — Step 编写契约
