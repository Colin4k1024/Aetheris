...
| **Step 至多执行一次**（同一逻辑步不重复已记录副作用） | 是 | 配置 **InvocationLedger**（及共享 ToolInvocationStore）时，同一 idempotency_key 仅允许一次 Commit；Replay 从事件流/Ledger 注入结果，不再次执行。配置 **Effect Store** 时采用两步提交（先 PutEffect 再 Append），崩溃恢复从 Effect Store catch-up，不重执行。见 [effect-store-contract.md](internal/effect-store-contract.md)、[effect-system.md](internal/effect-system.md)、[scheduler-correctness.md](internal/scheduler-correctness.md)。 |
...
- **无全局事务**：Ledger、Effect Store 与事件提交**不在同一 DB 事务**中；保证依赖明确写入顺序（Tool started → Execute → PutEffect → Append committed/finished → Ledger.Commit）与 **Effect Store catch-up**。可证明语义（crash 窗口、幂等键、顺序与补偿）见 [effect-store-contract.md](internal/effect-store-contract.md) 与 [provable-semantics-table.md](internal/provable-semantics-table.md)。

---

## 参考

- [effect-store-contract.md](internal/effect-store-contract.md) — 当前 Effect Store 写入顺序与恢复契约
- [effect-system.md](internal/effect-system.md) — 效应类型、Replay 协议、Effect Store 与两步提交
- [scheduler-correctness.md](internal/scheduler-correctness.md) — 租约、两步提交、Step 状态
- [runtime-contract.md](internal/runtime-contract.md) — 重放边界、Blocking、租约、PlanGenerated、attempt_id
- [1.0-runtime-semantics.md](internal/1.0-runtime-semantics.md) — Execution Proof Chain、Ledger 决策、World Safety
- [provable-semantics-table.md](internal/provable-semantics-table.md) — 可证明语义表（crash 窗口、幂等键、Ledger/Append 顺序）
- [agent-process-model.md](internal/agent-process-model.md) — Signal、Mailbox、External Event 交付
