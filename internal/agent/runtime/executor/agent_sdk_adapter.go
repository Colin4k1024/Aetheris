...
// AgentSDKClient Agent SDK Go 桥接接口
type AgentSDKClient interface {
	CreateAgent(ctx context.Context, config map[string]any) (interface {
		Run(ctx context.Context, input string) (string, error)
	}, error)
}

// AgentSDKNodeAdapter 将 Agent SDK 型 TaskNode 转为 DAG 节点
type AgentSDKNodeAdapter struct {
	Client           AgentSDKClient
	CommandEventSink CommandEventSink
	EffectStore      EffectStore
}

func (a *AgentSDKNodeAdapter) runNode(ctx context.Context, taskID string, cfg map[string]any, p *AgentDAGPayload) (*AgentDAGPayload, error) {
	if a.Client == nil {
		return nil, fmt.Errorf("AgentSDKNodeAdapter: Client not configured")
	}
	if p == nil {
		p = &AgentDAGPayload{}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("AgentSDKNodeAdapter: failed to create agent: %w", err)
	}

	if a.CommandEventSink != nil && jobID != "" {
		inputBytes, _ := json.Marshal(map[string]any{"input": p.Goal})
		_ = a.CommandEventSink.AppendCommandEmitted(ctx, jobID, taskID, taskID, "agent_sdk", inputBytes)
	}
	...
	if a.EffectStore != nil && jobID != "" {
		...
		_ = a.EffectStore.PutEffect(ctx, &EffectRecord{
			...
			Metadata: map[string]any{"adapter": "agent_sdk"},
		})
	}
...
func (a *AgentSDKNodeAdapter) ToDAGNode(task *planner.TaskNode, _ *runtime.Agent) (*compose.Lambda, error) {
	taskID := task.ID
	cfg := task.Config
	if cfg == nil {
		cfg = map[string]any{}
...
func (a *AgentSDKNodeAdapter) ToNodeRunner(task *planner.TaskNode, _ *runtime.Agent) (NodeRunner, error) {
	taskID := task.ID
	cfg := task.Config
	if cfg == nil {
		cfg = map[string]any{}
