...
// ProtocolLatticeClient Protocol-Lattice 桥接接口
type ProtocolLatticeClient interface {
	CreateAgent(ctx context.Context, config map[string]any) (interface {
		Execute(ctx context.Context, input string) (string, error)
	}, error)
}

// ProtocolLatticeNodeAdapter 将 Protocol-Lattice 型 TaskNode 转为 DAG 节点
type ProtocolLatticeNodeAdapter struct {
	Client           ProtocolLatticeClient
	CommandEventSink CommandEventSink
	EffectStore      EffectStore
}

func (a *ProtocolLatticeNodeAdapter) runNode(ctx context.Context, taskID string, cfg map[string]any, p *AgentDAGPayload) (*AgentDAGPayload, error) {
	if a.Client == nil {
		return nil, fmt.Errorf("ProtocolLatticeNodeAdapter: Client not configured")
	}
	if p == nil {
		p = &AgentDAGPayload{}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ProtocolLatticeNodeAdapter: failed to create agent: %w", err)
	}

	if a.CommandEventSink != nil && jobID != "" {
		inputBytes, _ := json.Marshal(map[string]any{"input": p.Goal})
		_ = a.CommandEventSink.AppendCommandEmitted(ctx, jobID, taskID, taskID, "protocol_lattice", inputBytes)
	}

	output, err := agent.Execute(ctx, p.Goal)
	if err != nil {
		if p.Results == nil {
			p.Results = make(map[string]any)
		}
	...
	if a.EffectStore != nil && jobID != "" {
		...
		_ = a.EffectStore.PutEffect(ctx, &EffectRecord{
			...
			Metadata: map[string]any{"adapter": "protocol_lattice"},
		})
	}
...
func (a *ProtocolLatticeNodeAdapter) ToDAGNode(task *planner.TaskNode, _ *runtime.Agent) (*compose.Lambda, error) {
	taskID := task.ID
	cfg := task.Config
	if cfg == nil {
		cfg = map[string]any{}
...
func (a *ProtocolLatticeNodeAdapter) ToNodeRunner(task *planner.TaskNode, _ *runtime.Agent) (NodeRunner, error) {
	taskID := task.ID
	cfg := task.Config
	if cfg == nil {
		cfg = map[string]any{}
