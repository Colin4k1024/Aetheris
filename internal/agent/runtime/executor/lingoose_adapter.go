...
// LinGooseClient LinGoose 桥接接口
type LinGooseClient interface {
	CreateAgent(ctx context.Context, config map[string]any) (interface {
		Run(ctx context.Context, input string) (string, error)
	}, error)
}

// LinGooseNodeAdapter 将 LinGoose 型 TaskNode 转为 DAG 节点
type LinGooseNodeAdapter struct {
	Client           LinGooseClient
	CommandEventSink CommandEventSink
	EffectStore      EffectStore
}

func (a *LinGooseNodeAdapter) runNode(ctx context.Context, taskID string, cfg map[string]any, p *AgentDAGPayload) (*AgentDAGPayload, error) {
	if a.Client == nil {
		return nil, fmt.Errorf("LinGooseNodeAdapter: Client not configured")
	}
	if p == nil {
		p = &AgentDAGPayload{}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("LinGooseNodeAdapter: failed to create agent: %w", err)
	}

	if a.CommandEventSink != nil && jobID != "" {
		inputBytes, _ := json.Marshal(map[string]any{"input": p.Goal})
		_ = a.CommandEventSink.AppendCommandEmitted(ctx, jobID, taskID, taskID, "lingoose", inputBytes)
	}
	...
	if a.EffectStore != nil && jobID != "" {
		...
		_ = a.EffectStore.PutEffect(ctx, &EffectRecord{
			...
			Metadata: map[string]any{"adapter": "lingoose"},
		})
	}
...
func (a *LinGooseNodeAdapter) ToDAGNode(task *planner.TaskNode, _ *runtime.Agent) (*compose.Lambda, error) {
	taskID := task.ID
	cfg := task.Config
	if cfg == nil {
		cfg = map[string]any{}
...
func (a *LinGooseNodeAdapter) ToNodeRunner(task *planner.TaskNode, _ *runtime.Agent) (NodeRunner, error) {
	taskID := task.ID
	cfg := task.Config
	if cfg == nil {
		cfg = map[string]any{}
