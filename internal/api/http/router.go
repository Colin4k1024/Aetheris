...
func (r *Router) Build(addr string, opts ...config.Option) *server.Hertz {
	...
	agents.Use(r.middleware.Legacy("/api/runs + /api/jobs"))
	{
		...
		agents.GET("/:id/approvals", r.authChainWith(auth.PermissionJobView, r.handler.ListAgentApprovals)...)
	...
	jobs := api.Group("/jobs")
	{
		...
		jobs.GET("/:id/approval", r.authChainWith(auth.PermissionJobView, r.handler.GetJobApproval)...)
		jobs.POST("/:id/approval/approve", r.authChainWith(auth.PermissionJobCreate, r.handler.ApproveJobApproval)...)
		jobs.POST("/:id/approval/reject", r.authChainWith(auth.PermissionJobCreate, r.handler.RejectJobApproval)...)
		jobs.POST("/:id/approval/delegate", r.authChainWith(auth.PermissionJobCreate, r.handler.DelegateJobApproval)...)
		...
		jobs.GET("/:id/ledger", r.authChainWith(auth.PermissionTraceView, r.handler.GetJobLedger)...)
