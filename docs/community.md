# Aetheris Community

Join our community to connect with other developers, get help, and share your projects.

---

## Community Platforms

### Discord (Recommended)

Join our Discord server for real-time discussions:

**[Join Discord](https://discord.gg/aetheris)**

#### Channel Structure

| Channel | Description |
|---------|-------------|
| `#announcements` | Product updates, releases, news |
| `#general` | General discussion |
| `#introductions` | Introduce yourself |
| `#help` | Get help with Aetheris |
| `#show-and-tell` | Share your projects |
| `#feedback` | Feature requests, suggestions |
| `#jobs` | Job opportunities |

#### Roles

| Role | Description |
|------|-------------|
| `@admin` | Project maintainers |
| `@contributor` | Active contributors |
| `@user` | General members |

---

### GitHub Discussions

For asynchronous discussions and Q&A:

[**GitHub Discussions**](https://github.com/Colin4k1024/Aetheris/discussions)

Categories:
- **Q&A** - Ask and answer questions
- **Show and Tell** - Share your projects
- **Ideas** - Feature suggestions
- **General** - Other discussions

---

## Community Guidelines

1. **Be Respectful** - Treat everyone with respect
2. **Stay on Topic** - Keep discussions relevant
3. **No Spam** - No promotional content without permission
4. **Help Others** - Share your knowledge
5. **Follow Code of Conduct** - See [CODE_OF_CONDUCT.md](../CODE_OF_CONDUCT.md)

---

## Contributing

Want to contribute? Check out:

- [Contributing Guide](../CONTRIBUTING.md)
- [Good First Issues](https://github.com/Colin4k1024/Aetheris/labels/good%20first%20issue)
- [Development Guide](../CLAUDE.md)

---

## Events

### Upcoming

- **Aetheris Office Hours** - Monthly Q&A with maintainers
- **Community Call** - Bi-weekly updates

### Past

- v2.2.0 Release Announcement
- First Community Meetup

---

## Get Help

| Resource | Response Time |
|----------|---------------|
| Discord #help | ~24 hours |
| GitHub Discussions | ~48 hours |
| GitHub Issues | For bugs only |

---

## Newsletter

Subscribe for updates:

- RSS Feed: (coming soon)
- GitHub Stars: [Subscribe](https://github.com/Colin4k1024/Aetheris/stargazers)

---

*Last updated: 2026-03-04*
