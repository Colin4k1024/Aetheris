...
- 再启动 1 个 API + 2 个 Worker（见 [releases/release-certification-1.0.md](../releases/release-certification-1.0.md) 环境说明）。
...
CLI 用于调试与管理：runtime-first 以 Job/Run 为主；`agent/chat` 为兼容层命令。详见 [CLI (cli.md)](cli.md)。
...
更多配置与接口说明见 [usage.md](usage.md)、[config.md](../reference/config.md)；部署见 [deployment.md](deployment.md)；故障排查见 [troubleshooting.md](troubleshooting.md)。
...
| 创建 Agent（兼容层）    | POST /api/agents                       | 是       | 是         |
| 发消息得 Job（兼容层）  | POST .../message → 202 + job_id        | 是       | 是         |
