...
1. **Create agent (legacy facade)**: `aetheris agent create refund-agent` (or `POST /api/agents`).
2. **Send message (legacy facade)**: `aetheris chat <agent_id>` and enter e.g. “Refund order-123” (or `POST /api/agents/:id/message`).
