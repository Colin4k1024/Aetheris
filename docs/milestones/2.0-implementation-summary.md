...
- **Key Features**:
  ...
  - Verify-before-delete: GC deletes source rows only after the archived copy is written **and** read back successfully
  - Fail-closed: archive sink unavailable / not configured / verification mismatch aborts GC without deleting source rows
  - Idempotent re-runs: archive PK matches source PK, so crash recovery never duplicates or loses copies
  - Config: `runtime.gc.archive_enabled`, `runtime.gc.archive_ttl_days` (0 = keep archived copies forever)

> Implementation: `internal/runtime/jobstore/archive.go` (+ `gc.go`, `schema.sql`).
> The archive path was a documented no-op before issue #226 item 01; it is now backed by real persistence and tests.
