...
> 状态口径说明：当前发布状态与能力成熟度以 [../STATUS.md](../STATUS.md) 为准；本文档主要用于路线与历史规划参考。
...
| Observability | 已增强 | CountPending、ListStuckRunningJobIDs、GET /api/observability/summary。`internal/agent/job/observability.go`、[guides/observability.md](../guides/observability.md) |
...
- **Step Contract Runtime Enforcement** — 已实现 SDK 与文档；待强化：Runner 在调用 step 前注入 `effects.WithRecordedEffects` 与 `sdk.WithRuntimeContext`，限制 step 仅能使用 runtime.Tool() / runtime 时间/UUID/HTTP。见 [design/step-contract.md](../design/step-contract.md)、[guides/sdk.md](../guides/sdk.md)。
...
- **Job Health & Debugging** — 已增强：Stuck job 检测（ListStuckRunningJobIDs）、Queue backlog（CountPending）、GET /api/observability/summary。见 [guides/observability.md](../guides/observability.md)。可选下一步：Stuck Job UI、Retry 时间线可视化。
...
| 低 | Observability & SRE UI | 运维可见性提升 | `internal/agent/job/observability.go`、[guides/observability.md](../guides/observability.md)；可选 Stuck Job UI、Retry 时间线 |
...
- [guides/observability.md](../guides/observability.md) — 可观测性 API 与指标
- [guides/sdk.md](../guides/sdk.md) — Step Programming Model 与 SDK
